...
class MovieSearchResultViewController: UITableViewController, XMLParserDelegate {
    @IBOutlet weak var titleNavigationItem: UINavigationItem!
    
    let posterImageQueue = DispatchQueue(label: "posterImage")
    
    let clientID        = "OYrOZ6J1jjH9_F3hJuin"    // ClientID
    let clientSecret    = "pkbk6rhxSa"              // ClientSecret
    
    var queryText:String?                   // SearchVC에서 받아 오는 검색어
    var movies:[MovieSearchData]              = []    // API를 통해 받아온 결과를 저장할 array
    
    // XML delegate
    var strXMLData: String?         = ""   // xml 데이터를 저장
    var currentTag: String?         = ""   // 현재 item의 tag를 저장
    var currentElement: String      = ""   // 현재 element의 내용을 저장
    var item: MovieSearchData?                = nil  // 검색하여 만들어지는 Movie 객체
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ListCell", for: indexPath) as! MovieSearchCell
